    ...
    pub fn into_parts(self) -> RegistryParts {
        ...
        for entry in self.entries {
            ...
            for &related in entry.related_routes {
                debug_assert!(
                    !route_labels.contains_key(&related),
                    "Duplicate related route: {:?}",
                    related
                );
...
pub fn register_all_modules(
    ...
    use crate::module::{
        flavor::FlavorModule, floating_ip::FloatingIpModule, host::HostModule, image::ImageModule,
        network::NetworkModule, project::ProjectModule, security_group::SecurityGroupModule,
        server::ServerModule, snapshot::SnapshotModule, usage::UsageModule, user::UserModule,
        volume::VolumeModule,
    };
...
mod tests {
    ...
    fn test_registry_into_parts_route_labels() {
        ...
        assert_eq!(
            parts.route_labels.get(&Route::ServerDetail),
            Some(&"Servers")
        );
        assert_eq!(
            parts.route_labels.get(&Route::ServerCreate),
            Some(&"Servers")
        );
    }
