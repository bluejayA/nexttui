...
impl UserModule {
    ...
    fn selected_user(&self) -> Option<&User> {
        self.users.get(self.resource_list.selected_index())
    }
    fn rows(&self) -> Vec<Row> {
        self.users.iter().map(user_to_row).collect()
    }
        ...
        match form.handle_key(key) {
            FormAction::Submit(values) => {
                ...
                let password = values
                    ...
                    .unwrap_or_default();
                let email = values.get("Email").and_then(|v| match v {
    ...
    fn handle_event(&mut self, event: &AppEvent) {
        match event {
            AppEvent::UsersLoaded(users) => {
                ...
                let mut domain_ids: Vec<String> =
                    users.iter().filter_map(|u| u.domain_id.clone()).collect();
                domain_ids.sort();
                domain_ids.dedup();
...
mod tests {
    ...
    fn setup() -> (UserModule, ActionReceiver) {
        ...
        m.handle_event(&AppEvent::UsersLoaded(vec![
            make_user("u1", "admin"),
            make_user("u2", "demo"),
        ]));
        (m, rx)
