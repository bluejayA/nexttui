...
pub fn compute_service_columns() -> Vec<ColumnDef> {
    vec![
        ...
        ColumnDef { name: "Disabled Reason".into(), width: ColumnWidth::Percent(25), alignment: ratatui::layout::Alignment::Left },
    ]
}
...
pub fn compute_service_to_row(svc: &ComputeService) -> Row {
    ...
    Row {
        id: svc.id.clone(),
        cells: vec![svc.binary.clone(), svc.host.clone(), svc.state.clone(), svc.status.clone(), reason.to_string()],
...
mod tests {
    use super::*;
    #[test] fn test_columns() { assert_eq!(compute_service_columns().len(), 5); }
    #[test] fn test_to_row() {
        let svc = ComputeService { id: "s1".into(), binary: "nova-compute".into(), host: "n1".into(), state: "up".into(), status: "enabled".into(), updated_at: None, disabled_reason: None };
